 ...
 * Details
 */
public class IOSLive extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ios_live);

        AsyncHttpClient client = new AsyncHttpClient();
        client.get("https://api.myjson.com/bins/387vo", new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                ...
                ArrayList<String> status = getStatus(response, "countryList", "login", "fbcard", "addBooking", "checkin", "bpass", "time", "finalStatus");
                ...
                if(status.get(6)!=null) {
                    r.setText("Status @"+status.get(6));
                }
                if(status.get(7)!=null) {
                    String[] details = status.get(7).split(",");
